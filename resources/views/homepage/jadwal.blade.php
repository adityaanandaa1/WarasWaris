<section id="schedule">
    <div class="schedule-header">
        <h1 class="schedule-title" data-aos="fade-down">Jadwal Klinik</h1>
        <h2 class="schedule-subtitle" data-aos="fade-down">Dr. Muh. Abd. Waris</h2>
    </div>

    <div class="schedule-box" data-aos="fade-down">
        <div class="schedule-info">
            <p class="info-title">Jam Praktik</p>
            
            <form method="GET" action="{{ route('homepage') }}#schedule" id="scheduleForm">
                <div class="date-picker">
                    <label for="myDate" style="cursor: pointer;">
        ...
        <div class="schedule-time">
            @if($jadwal && $jadwal->is_active)
                <div class="time-open">
                    ...
                    <h1 class="time-value">
                        {{ $jadwal->jam_mulai ? \Carbon\Carbon::parse($jadwal->jam_mulai)->format('H:i') : '09:00' }}
                    </h1>
                </div>
                ...
                <div class="time-close">
                    ...
                    <h1 class="time-value">
                        {{ $jadwal->jam_selesai ? \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') : '21:00' }}
                    </h1>
                </div>
            @else
                <div class="time-close" style="width: 100%; text-align: center;">
                    <h3 class="time-label" data-aos="fade-down">Status</h3>
                    <h1 class="time-value" data-aos="fade-down" style="color: #ff6b6b;">LIBUR</h1>
                    <p data-aos="fade-down" style="color: #fff; margin-top: 10px; font-size: 14px;">
                        Klinik tutup pada {{ $nama_hari }}, {{ $tanggal_dipilih->format('d/m/Y') }}
                    </p>
                </div>
            @endif
        </div>
...
<script>
    // Update datepicker untuk submit form saat tanggal dipilih
    document.addEventListener('DOMContentLoaded', function() {
        const dateInput = document.getElementById('myDate');
        const scheduleForm = document.getElementById('scheduleForm');
        const calendarIcon = document.querySelector('.date-picker svg'); 
        
        if (dateInput && scheduleForm) {
            flatpickr(dateInput, {
                dateFormat: "Y-m-d",
                defaultDate: "{{ $tanggal_dipilih->format('Y-m-d') }}",
                locale: "id",
